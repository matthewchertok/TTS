...
class TTSDataset(Dataset):
    ...
    def load_wav(self, filename):
        try:
            filename = filename.replace('"', "")  # remove any apostrophe characters
            filename = filename.replace('\ufeff', '')  # remove any \ufeff characters
    ...
    def _compute_lengths(samples):
        new_samples = []
        for item in samples:
            audio_file_path = item["audio_file"]
            try:
                audio_file_path = audio_file_path.replace('"', "")  # remove any apostrophe characters
                audio_file_path = audio_file_path.replace('\ufeff', '')  # remove any \ufeff characters
            except Exception as e:
                print(e)
            audio_length = os.path.getsize(audio_file_path) / 16 * 8  # assuming 16bit audio
